...
class Episode
  ...
  attr_reader :id, :published, :published_rfc, :updated, :dj, :title, :slug, :brand, :description, :track_list,
              :duration, :genre, :labels, :image_url, :audio_url, :file_size, :downloads

  # Initialize properties
  def initialize(entry)
    @id             = entry.sys[:id]
    @published      = Date.strptime(entry.fields[:release_date].tr('-', ''), '%Y%m%d')
    @published_rfc  = DateTime.parse(entry.fields[:release_date]).rfc2822
